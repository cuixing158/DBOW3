...
void convertCVToMatrix(cv::Mat &srcImg, int rows, int cols, int channels, unsigned char *dst[]) {
    //CV_Assert(srcImg.total() * 3 == 589824U && srcImg.type() == CV_32FC3);
    CV_Assert(srcImg.type() == CV_8U);
...
void convertToMat(const unsigned char *inImg[], int rows, int cols, int channels, cv::Mat &matBigImg) {
    ...
    if (channels == 3) {
        cv::Mat matR = cv::Mat(cols, rows, CV_8UC1, inImg);
        cv::Mat matG = cv::Mat(cols, rows, CV_8UC1, inImg + elems);
        cv::Mat matB = cv::Mat(cols, rows, CV_8UC1, inImg + 2 * elems);
        std::vector<cv::Mat> matBGR = {matB.t(), matG.t(), matR.t()};
        cv::merge(matBGR, matBigImg);
    } else {
        cv::Mat matBigImg = cv::Mat(cols, rows, CV_8UC1, inImg);
    }
}
...
void imwarp(const cv::Mat srcImg, int rows, int cols, int channels, float tformA[], imref2d outputView, cv::Mat &outImg) {
    ...
    cv::Mat transMat = (cv::Mat_<float>(2, 3) << tformA[0], tformA[3], tformA[6],
    ...
    // 平移到可视化区域
    transMat.colRange(2, 3) = transMat.colRange(2, 3) - (cv::Mat_<float>(2, 1) << outputView.XWorldLimits[0], outputView.YWorldLimits[0]);

    cv::warpAffine(srcImg, outImg, transMat, cv::Size(outputView.ImageSize[1], outputView.ImageSize[0]));
}

void imwarp2(const unsigned char *inImg[], int rows, int cols, int channels, double *tformA[9], imref2d *outputView, unsigned char *outImg[]) {
    ...
    {
        cv::Mat transMat = (cv::Mat_<float>(3, 3) << tformA[0], tformA[3], tformA[6],
                            tformA[1], tformA[4], tformA[7],
                            tformA[2], tformA[5], tformA[8]);
        // 平移到可视化区域
        transMat.colRange(2, 3) = transMat.colRange(2, 3) - (cv::Mat_<float>(3, 1) << outputView->XWorldLimits[0], outputView->YWorldLimits[0], 1.0);
